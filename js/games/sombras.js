const GameSombras = {
  ...
  loadQuestion() {
    ...
    const optsHTML = opts.map(o =>
      `<button class="shadow-opt" id="sho-${o.codePointAt(0)}" onclick="GameSombras.answer('${o}','${q.answer}','${q.label}')">${o}</button>`
    ).join('');

    document.getElementById('sh-area').innerHTML = `
      <div class="shadow-wrap">
        <div class="shadow-question">¿Qué animal es esta sombra?</div>
        <div class="shadow-silhouette" id="sh-sil">${q.answer}</div>
  ...
  answer(val, correct, label) {
    ...
    const btn = document.getElementById('sho-'+val.codePointAt(0));
    if (btn) btn.classList.add(ok ? 'correct-opt' : 'wrong-opt');
    if (!ok) {
      const correctBtn = document.getElementById('sho-'+correct.codePointAt(0));
      if (correctBtn) correctBtn.classList.add('correct-opt');
    }
    // Reveal silhouette
    const sil = document.getElementById('sh-sil');
    if (sil) sil.classList.add('reveal');
