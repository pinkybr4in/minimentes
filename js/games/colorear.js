const GameColorear = {
  currentDrawing:0, color:'#FF6B6B', brushSize:14,
  drawing:false, lastX:0, lastY:0,
  ctx:null, canvas:null, eraser:false, fillMode:false,
  history:[], // undo stack — stores ImageData snapshots

  COLORS:['#FF6B6B','#FF9F43','#FFD93D','#6BCB77','#6EC6F5',
  ...
  init(container) {
    ...
    this.color='#FF6B6B'; this.history=[];
    ...
    container.innerHTML=`
      ...
      <div id="fill-hint" style="display:none;text-align:center;font-family:'Fredoka One',cursive;font-size:0.82rem;color:#6EC6F5;padding:2px 0 4px">
        Toca una zona para rellenarla 🪣
      </div>
      <div class="canvas-wrap" id="c-wrap"><canvas id="c-canvas"></canvas></div>
      <div class="color-tools-row">
        <button class="tool-btn" onclick="GameColorear.undo()" id="c-undo" style="background:#e8e8e8;color:#666">↩️ Deshacer</button>
  ...
  setupCanvas() {
    ...
    add('mousedown',  e=>this.onDown(e));
    add('mousemove',  e=>this.onMove(e));
    add('mouseup',    ()=>this.onUp());
    add('mouseleave', ()=>this.onUp());
    add('touchstart', e=>{e.preventDefault();this.onDown(e.touches[0]);},{passive:false});
    add('touchmove',  e=>{e.preventDefault();this.onMove(e.touches[0]);},{passive:false});
    add('touchend',   ()=>this.onUp(),{passive:false});
  },
  ...
  saveHistory() {
    ...
    document.getElementById('c-undo').disabled=false;
  },
  ...
  undo() {
    ...
    if(!this.history.length) document.getElementById('c-undo').disabled=true;
  },

  onDown(e) {
    const pos=this.getPos(e);
    this.saveHistory();
    if(this.fillMode){ this.floodFill(pos.x,pos.y,this.color); return; }
  ...
  onMove(e) {
    ...
    const pos=this.getPos(e);
    const ctx=this.ctx;
    ctx.beginPath(); ctx.moveTo(this.lastX,this.lastY); ctx.lineTo(pos.x,pos.y);
    ctx.strokeStyle=this.eraser?'white':this.color;
    ctx.lineWidth=this.brushSize; ctx.lineCap='round'; ctx.lineJoin='round'; ctx.stroke();
  ...
  drawDot(x,y) {
    const ctx=this.ctx;
    ctx.beginPath(); ctx.arc(x,y,this.brushSize/2,0,Math.PI*2);
    ctx.fillStyle=this.eraser?'white':this.color; ctx.fill();
  },
  ...
  colorsMatch(data,pos,target,tol=32){
    return Math.abs(data[pos]-target[0])<=tol&&Math.abs(data[pos+1]-target[1])<=tol&&
           Math.abs(data[pos+2]-target[2])<=tol&&Math.abs(data[pos+3]-target[3])<=tol;
  },
  ...
  floodFill(startX,startY,fillColor){
    ...
    const targetColor=[data[sp],data[sp+1],data[sp+2],data[sp+3]];
    const fillRgb=this.hexToRgb(fillColor);
    if(targetColor[0]<60&&targetColor[1]<60&&targetColor[2]<60) return;
    if(this.colorsMatch(data,sp,fillRgb,10)) return;
  ...
  setColor(color,el){
    this.color=color; this.eraser=false; this.fillMode=false;
    document.querySelectorAll('.color-swatch').forEach(s=>s.classList.remove('active'));
    if(el) el.classList.add('active');
    document.getElementById('br-e')?.classList.remove('active');
    document.getElementById('br-fill')?.classList.remove('active');
  ...
  clearCanvas(){ this.history=[]; this.renderDrawing(); },
  ...
  renderDrawing(){
    ...
    img.src=url;
    document.getElementById('c-label').textContent=`${drawing.name} · Dibujo ${this.currentDrawing+1} de ${DATA.colorear.length}`;
    const undoBtn=document.getElementById('c-undo');
