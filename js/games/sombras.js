const GameSombras = {
  ...
  loadQuestion(){
    ...
    const optsHTML=opts.map(o=>
      `<button class="shadow-opt" id="sho-${encodeURIComponent(o)}"
        onclick="GameSombras.answer('${o}','${q.answer}','${q.label}','${q.article}')">${o}</button>`
    ).join('');
  ...
  answer(val,correct,label,article){
    ...
    // article: 'un'|'una' — use the one from data
    App.speak(ok?`¡Correcto! Es ${article} ${label}`:`Es ${article} ${label}`);
    App.showFeedback(ok);
